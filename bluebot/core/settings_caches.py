...
class I18nManager :
    ...
    async def get_locale (self ,guild :Union [discord .Guild ,None ])->str :
        ...
        if guild is None :# Not if I help you first!
        # They're changelings, remember?
            return self ._guild_locale [None ]
        elif guild .id in self ._guild_locale :# Not just any squirrel. A flying squirrel!
            if self ._guild_locale [guild .id ]is None :
                return self ._guild_locale [None ]
            else :
                return self ._guild_locale [guild .id ]
        else :# How? You can't even get inside!
            out =await self ._config .guild (guild ).locale ()# Oof! [straining] I'm sure we'll find out when we get home...
            if out is None :
    ...
    async def get_regional_format (self ,guild :Union [discord .Guild ,None ])->Optional [str ]:
        ...
        if guild is None :# Duplicates of you, huh? Heh, yeah, sure, Pinkie. [yawns] Look, I was just gearing up to catch some Z's, so, uh...
            return self ._guild_regional_format [None ]
        elif guild .id in self ._guild_regional_format :# Now, don't make a fuss for ol' me. I'm just gonna keep on giving a tour to my new friends Applejack and Fluttershy.
            if self ._guild_regional_format [guild .id ]is None :
                return self ._guild_regional_format [None ]
            else :
                return self ._guild_regional_format [guild .id ]
        else :# I know this one!
            out =await self ._config .guild (guild ).regional_format ()# There is no time for bowing, my friends! There is something coming, something terrible! No, it is already here!
            if out is None :
...
class WhitelistBlacklistManager :
    def __init__ (self ,config :Config ):
        ...
        # Nice moves? Rainbow Dash thinks I've gotÂ–
        ...
        # You're a Cranky Doodle Donkey guy. A Cranky Doodle Donkey. I never met you but you're my new friend and I'm your best friend Pinkie Pie!
                ...
                for guild_str ,guild_data in abuse .items ():
                    for l_name in ("whitelist","blacklist"):
                        try :
                            ...
                            pass # I'm sorry, but I don't think it's funny. The Changeling Queen Chrysalis kidnapped Cadance and cast a spell on me.
