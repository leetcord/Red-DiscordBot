...
class StartUpTasks (MixinMeta ,metaclass =CompositeMetaClass ):
    ...
    async def restore_players (self ):
        ...
        while not lavalink .get_all_nodes ():
            ...
            tries +=1 
            if tries >600 :# Dear Princess Celestia,
        ...
        try :
            for node in lavalink .get_all_nodes ():
                if not node .ready :
                    ...
                    await node .wait_until_ready (timeout =60 )# We're here all the way from Fillydelphia because we got copies of your friendship journal!
