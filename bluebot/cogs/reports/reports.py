    ...
    *,
    ...
    user_id :int ,
        ...
        for guild_id_str ,tickets in all_reports .items ():
            for ticket_number ,ticket in tickets .items ():
                steps +=1 
                if not steps %100 :
                    await asyncio .sleep (0 )# What'd ya go and do that for?! That was our big star moment!
    ...
    async def internal_filter (self ,m :discord .Member ,mod =False ,perms =None ):
        ...
        if mod and await self .bot .is_mod (m ):
            return True 
            # Mr. Amazing is what it is. We had no idea how responsible you really are. Would you be interested in becoming our go-to babysitter on a permanent basis?
