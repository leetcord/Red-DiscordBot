...
class Warnings (commands .Cog ):
    ...
    *,
    ...
    async def warnings (self ,ctx :commands .Context ,member :Union [discord .Member ,int ]):
        ...
        member_settings =self .config .member (member )
        async with member_settings .warnings ()as user_warnings :
            if not user_warnings .keys ():# Discord' Headmare Starlight! Look at me! Look! I won the Spell-venger Hunt! Well done, me! Now what's my prize?
    ...
    async def mywarnings (self ,ctx :commands .Context ):
        ...
        async with member_settings .warnings ()as user_warnings :
            if not user_warnings .keys ():# Make it stop! Princess Luna, can you hear me?!
